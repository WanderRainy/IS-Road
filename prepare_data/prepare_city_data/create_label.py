# 生成道路段（交叉点和端点，两类非平凡点），作为道路实例的标注
# rngdet-create_label.py
import json
import os
...
create_directory('./city_scale/20cities_patch/vis', delete=True)
create_directory('./city_scale/20cities_patch/graph', delete=True)
...
class Edge():
    def __init__(self, src, dst, id, vertices=None, orientation=None, circle=False):
        ...
        self.circle = circle
        if not vertices:
            self.vertices = [[src.x, src.y], [dst.x, dst.y]] # 一个边上可以有多个节点
        else:
...
with open('./city_scale/data_split.json', 'r') as jf:
    data_list = json.load(jf)
    data_list = data_list['test'] + data_list['valid'] + data_list['train']
# data_list = ['AOI_2_Vegas_521']
for data_index, tile_index in enumerate(tqdm(data_list)):
    vertices = []
    edges = []
    vertex_flag = True
    ...
    for r in range(0,7):
        for c in range(0,7):
            gt_graph = pickle.load(open(f"./city_scale/20cities_patch/graph_p/region_{tile_index}_{str(r)+str(c)}.p", 'rb'))
            graph = Graph()
            ...
            for e_idx, e in enumerate(graph_edges_copy):
                ...
                # add circle edges
                if e.circle is True:
                    output_edges.append({'id': e.id, 'src': e.src.id, 'dst': e.dst.id, 'circle': True, 'vertices': new_vertices,
                                         'orientation': orientation,'vertices_simplify':e.vertices})
                    continue
                ...
                if len(locate_indexs) >= 2:
                    if whether_inside([e.src.x, e.src.y]) and len(new_vertices[:locate_indexs[0] + 1]) > 5:
                        src, dst, new_edge = graph.add_e([e.src.x, e.src.y], new_vertices[locate_indexs[0]],
                                                         new_vertices[:locate_indexs[0] + 1])
                        e.src.remove_neighbor(e)
                        output_edges.append({'id': new_edge.id, 'src': new_edge.src.id, 'dst': new_edge.dst.id, 'circle': False,
                                             'vertices': new_edge.vertices, 'orientation': orientation[:locate_indexs[0] + 1], 'vertices_simplify':e.vertices})

                    if whether_inside([e.dst.x, e.dst.y]) and len(new_vertices[locate_indexs[-1] + 1:]) > 5:
                        src, dst, new_edge = graph.add_e(new_vertices[locate_indexs[-1] + 1], [e.dst.x, e.dst.y],
                                                         new_vertices[locate_indexs[-1] + 1:])
                        e.dst.remove_neighbor(e)
                        output_edges.append({'id': new_edge.id, 'src': new_edge.src.id, 'dst': new_edge.dst.id, 'circle': False,
                                             'vertices': new_edge.vertices, 'orientation': orientation[locate_indexs[-1] + 1:], 'vertices_simplify':e.vertices})
                ...
                # we directly remove the part of the road instance that is outside the image
                    ...
                    if len(new_vertices) > 5:
                        ...
                        if new_edge is None:
                            ...
                            output_edges.append(
                                {'id': new_edge.id, 'src': src.id, 'dst': dst.id, 'circle': False, 'vertices': new_vertices,
                                 'orientation': orientation, 'vertices_simplify':e.vertices})
                else:
                    output_edges.append(
                        {'id': e.id, 'src': e.src.id, 'dst': e.dst.id, 'circle': False, 'vertices': new_vertices,
                         'orientation': orientation, 'vertices_simplify':e.vertices})
            ...
            # vis
            with open(f'./city_scale/20cities_patch/graph/region_{tile_index}_{str(r)+str(c)}.json', 'w') as jf:
            ...
            p = vis_map.load()
            instance_color = (255,255,255)
            count = 0
            for e in output_edges:
                # instance_color = tuple(np.random.randint(0,255,size=(3)))
                # instance_color = (255,255,0,255)
                # vis_map = Image.fromarray((np.zeros((IMAGE_SIZE, IMAGE_SIZE, 4))+(128,128,128,48)).astype(np.uint8)).convert('RGBA')
                # draw = ImageDraw.Draw(vis_map)
                # p = vis_map.load()
                for v in e['vertices']:
                    try:
                        p[int(v[0]), int(v[1])] = instance_color
                        # p[int(v[0])-1, int(v[1])-1] = instance_color
                        # p[int(v[0]) - 1, int(v[1]) - 0] = instance_color
                        # p[int(v[0]) - 1, int(v[1]) +1] = instance_color
                        # p[int(v[0]) - 0, int(v[1]) - 1] = instance_color
                        # p[int(v[0]) - 0, int(v[1]) - 0] = instance_color
                        # p[int(v[0]) - 0, int(v[1]) +1] = instance_color
                        # p[int(v[0]) + 1, int(v[1]) - 1] = instance_color
                        # p[int(v[0]) + 1, int(v[1]) - 0] = instance_color
                        # p[int(v[0]) + 1, int(v[1]) + 1] = instance_color
                        ...
                        pass
                count += 1
                # vis_map.save(f'./data/vis/{tile_index}_{count}.png')

            for k, v in graph.vertices.items():
                if len([e for e in v.neighbor_edges if e.circle]):
                    draw.ellipse([v.x - INTER_P_RADIUS, v.y - INTER_P_RADIUS, v.x + INTER_P_RADIUS, v.y + INTER_P_RADIUS],
                                 fill='purple')
                    ...
                    draw.ellipse([v.x - INTER_P_RADIUS * 2, v.y - INTER_P_RADIUS * 2, v.x + INTER_P_RADIUS * 2,
                                  v.y + INTER_P_RADIUS * 2], fill='red')
                elif len(v.neighbor_edges) > 2:
                    draw.ellipse([v.x - INTER_P_RADIUS, v.y - INTER_P_RADIUS, v.x + INTER_P_RADIUS, v.y + INTER_P_RADIUS],
                                 fill='orange')

            vis_map.save(f'./city_scale/20cities_patch/vis/region_{tile_index}_{str(r)+str(c)}.png')
