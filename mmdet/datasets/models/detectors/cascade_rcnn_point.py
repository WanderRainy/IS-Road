...
class Cascade_PointRCNN(TwoStageDetector):
    ...
    def __init__(self,
        ...
        self.point_segmentation_branch = mmseg.models.decode_heads.FPNHead(feature_strides=[4, 8, 16, 32, 64],#[4, 8, 16, 32, 64],
                                                 in_channels=[256, 256, 256, 256, 256],#[256, 256, 256, 256, 256],
                                                 in_index=[0, 1, 2, 3, 4],#,4
                                                 channels=256,
                                                #  dropout_ratio=0.1,
                                                 num_classes=1,
                                                 threshold=0.5,
        ...
        # self.point_segmentation_branch = mmseg.models.decode_heads.FCNHead(
        #                                          in_channels=256,
        #                                          in_index=4,
        #                                          channels=128,
        #                                         #  dropout_ratio=0.1,
    ...
    def predict(self,
        ...
        point_result = F.interpolate(torch.sigmoid(point_result), (512,512), mode='bilinear', align_corners=False)
        # 保存point结果
        from PIL import Image
        import cv2
        target = 'work_dirs/exp7_1/point_city_out_vis/'
        import os
        ...
        for i in range(0, point_result.shape[0]):
            ...
            Image.fromarray(cv2.resize((predout.numpy()).transpose(1,2,0),(512,512),interpolation=cv2.INTER_LINEAR)).save(target + imgname + '.tif')
