...
class LoadAnnotationswithPointMask(MMCV_LoadAnnotations):
    """添加了读取点掩膜"""
    ...
    def __init__(
            ...
            **kwargs) -> None:
        super(LoadAnnotationswithPointMask, self).__init__(**kwargs)
    ...
    def transform(self, results: dict) -> dict:
        ...
        # 找到pointmask路径---road
        # cityes=['Vegas','Paris','Shanghai','Khartoum']
        # imgnameid = int(results['img_path'][-9:-4])
        # pointmaskpath = '/data1/yry22/Vector/RoadSegment/Point/data/pointmask/AOI_{}_{}_{}.png'.format(
        #     str(imgnameid//10000),cityes[imgnameid//10000-2],str(imgnameid%10000))
        # overpass
        # pointmaskpath = 'data/overpass/pointmasks/{}.png'.format(int(results['img_path'].split('/')[-1][:-4]))
        # results['gt_seg_map'] = mmcv.imfrombytes(
        #                 fileio.get(pointmaskpath),flag='unchanged').squeeze().astype(np.float32)/255
        # city_scale
        imgnameid = int(results['img_path'][-9:-4])
        # print(results['img_path'])
        pointmaskpath = '/data1/yry22/Vector/RoadSegment/mmdetection/data/city_scale/pointmask/region_{}_{}.png'.format(str(imgnameid//100),str(imgnameid%100).zfill(2))
        results['gt_seg_map'] = mmcv.imfrombytes(
                        fileio.get(pointmaskpath),flag='unchanged').squeeze().astype(np.float32)/255
        # if results['gt_seg_map'].max()!=1:
        #     print(results['gt_seg_map'].max())
        #     print(imgnameid)
